...
/*pub struct ConsumeLinkedListIter<T> {
    current:  Option<Box<Node<T>>>,
}

impl<T: Clone> IntoIterator for ConsumeLinkedListIter<T> {
    type Item = T;
    type IntoIter = ConsumeLinkedListIter<T>;
    fn into_iter(self) -> ConsumeLinkedListIter<T> {
        LinkedListIter {current: self.head}
    }
}*/
